...
export default function AddModel() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("يرجي ادخال قسم"),
      ...
      image_end_year: Yup.mixed()
        .required("يرجي ادخال صورة")
        .test("fileType", "يرجى رفع ملف صحيح", (value) => value != null),
      startYear: Yup.string().required("يرجي ادخال بدايه العام"),
      endYear: Yup.string().required("يرجي ادخال نهايه العام"),
  ...
  const deleteModel = async (id) => {
    try {
      const response = await axiosInstance.delete(`dashboard/models/${id}`, {
        headers: {
          Accept: "application/json, text/plain, */*",
        },
      });
      ...
      render: (
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <button
            ...
            onClick={() => deleteConfirmation(id, deleteModel)}
          >
  ...
  return (
    <>
      ...
      <Filtr
        data={ModelData}
        filtrated={setModelData}
        nameOfSession={"modelData"}
      />

      <div className="mt-3">
        {Array.isArray(ModelData) && (
          <Table
            pagination={ModelData?.length > 5 ? { pageSize: 5 } : false}
