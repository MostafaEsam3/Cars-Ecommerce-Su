...
export default function AddBrand() {
  ...
  const removeModaleAfterSubmit = () => {
    document
      .getElementById("deletemodalbrand")
  ...
  const deleteBrand = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `https://api.admin.kapitiano.com/dashboard/brands/${id}`,
        {
          headers: {
            Accept: "application/json, text/plain, */*",
            Authorization: `Bearer ${token}`,
          ...
          <img
            src={
              image.startsWith("http")
                ? image
                : `https://api.admin.kapitiano.com/storage/Images/Brands/${image}`
            }
            alt="avatar"
            style={{
              width: 30,
  ...
  return (
    <>
      ...
      <Filtr
        data={BrandData}
        filtrated={setBrandData}
        orig={BrandData}
        nameOfSession={"brandData"}
      />
      <div className="mt-3">
        {Array.isArray(BrandData) && (
          <Table
            pagination={BrandData?.length > 5 ? { pageSize: 5 } : false}
