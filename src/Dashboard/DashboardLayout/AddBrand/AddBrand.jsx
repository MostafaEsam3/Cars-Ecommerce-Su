...
export default function AddBrand() {
    ...
    const fetchBrand = async () => {
        await axios.get("http://127.0.0.1:8000/dashboard/brands").then((response) => {
            setBrandData(response.data.data);
            sessionStorage.setItem('brandData', JSON.stringify(response.data.data));
    ...
    const deleteBrand = async () => {
        try {
            const response = await axios.delete(
                `http://127.0.0.1:8000/dashboard/brands/${selectedBrandId}`,               {
                    headers: {
                       'Accept': 'application/json, text/plain, */*',
                    }
                }
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    "image":image,
                     id:id
                        ...
                        <button className="btn btn-danger ms-2" data-bs-toggle="modal" data-bs-target="#deletemodalbrand"  onClick={() => setSelectedBrandId(id)}> 
...
{/* component of filtration  */}
 <Filtr data={BrandData} filtrated={setBrandData} orig={BrandData} flagSession={"brand"}/>
<div className='mt-3'>
 <Table  pagination={BrandData.length > 5 ? { pageSize: 5 } :false} dataSource={BrandData} columns={columns} />
            ...
            <EditModal i={obj} typeOf={"brands"} idModal={"editBrand"} fetchBrand={fetchBrand} nameOfImage={"image"}/>
        </>
    );
}
