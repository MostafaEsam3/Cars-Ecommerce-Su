...
export default function AddProduct() {
  ...
  } = useFetchData("dashboard/categories", "categoryData");
  ...
  const addProduct = async (data) => {
    try {
      const response = await axiosInstance.post(
        "/dashboard/panelings",
        data, // هذا يجب أن يكون FormData
        {
          headers: {
            Accept: "application/json, text/plain, */*",
            // ⚠️ لا تضع Content-Type يدوياً هنا!
          },
        }
      );
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("يرجي ادخال منتج"),
      image: Yup.mixed()
      ...
      description: Yup.string().required("يرجى إدخال وصف صحيح"),
      category_id: Yup.string().required("يرجى إدخال اسم للقسم"),
    }),
  ...
  const deleteProduct = async (id) => {
    try {
      const response = await axiosInstance.delete(`dashboard/panelings/${id}`, {
        headers: {
          Accept: "application/json, text/plain, */*",
        },
      });
      ...
      render: (image, { name }) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          <img
            src={
              image
                ? image // هنا نأخذ الرابط مباشرة من الريسبونس
                : `https://api.admin.kapitiano.com/placeholder_images/default.svg` // صورة افتراضية إذا لم تكن موجودة
            }
            alt="avatar"
            style={{
              width: 30,
      ...
      render: (id, { name, image, description, status, link, category_id }) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <button
            ...
            onClick={() => deleteConfirmation(id, deleteProduct)}
          >
  ...
  return (
    <>
      ...
      <Filtr
        data={ProductData}
        filtrated={setProductData}
        nameOfSession={"panelingsData"}
      />
      ...
      <EditProduct
        i={obj}
        typeOf={"panelings"}
        idModal={"editPanelings"}
        fetchBrand={fetchProduct}
      />
    </>
  );
}
