...
const handleFilterAccCategory=async (id)=>{
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/products-by-category-id/${id}`);
...
useEffect(() => {
    ...
    return (
        <>
         <Category Data={CategoryData} handleFilterAccCategory={handleFilterAccCategory}  />
         <SecondHero returnAllProducts={returnAllProducts} />
<div className={Scroll?'wraber row justify-content-between mainFont':' row justify-content-between flex-wrap mainFont'}>
